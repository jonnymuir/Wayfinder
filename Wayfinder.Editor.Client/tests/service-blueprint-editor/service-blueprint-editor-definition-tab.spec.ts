...
test.describe('Definition (JSON twin-pane) tab', () => {
  ...
  test('Editing JSON to rename a stage updates the visual pane after debounce', async ({ page }) => {
    ...
    await expect(editor.locator('[data-wayfinder-stage="application-form"]')).toContainText('Renamed Form');
  });
  ...
  test('Visual change (rename a gateway) shows up in Definition tab', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const host = document.querySelector('wayfinder-service-blueprint-editor') as HTMLElement | null;
  ...
  test('Definition tab is keyboard-reachable and the editor accepts keyboard input', async ({ page }) => {
    ...
    await expect(editor.locator('[data-wayfinder-definition-panel]')).toBeVisible();
    const defEditor = editor.locator('[data-wayfinder-definition-editor]');
