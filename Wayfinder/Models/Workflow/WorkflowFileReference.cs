...
/// value in <c>WorkflowInstanceState.FieldValues</c>. Round-trips through JSON persistence —
...
public sealed record WorkflowFileReference
{
    ...
    /// </summary>
    public static WorkflowFileReference? FromFieldValue(object? raw) => raw switch
    {
        WorkflowFileReference reference => reference,
        JsonElement jsonElement when jsonElement.ValueKind == JsonValueKind.Object =>
            jsonElement.Deserialize<WorkflowFileReference>(new JsonSerializerOptions { PropertyNameCaseInsensitive = true }),
        _ => null
    };
}
