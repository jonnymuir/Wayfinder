...
/// <summary>
/// Validates a service request form submission against its authoritative field definitions —
/// field key whitelist, required, type coercion, options whitelist, and constraints. Every rule
/// here is derived purely from what the blueprint itself declares (<see cref="FieldRenderPayload"/>),
/// so it needs no per-host configuration and can never drift from what was actually rendered.
/// </summary>
public static class FieldValueValidator
{
    ...
    /// definitions.
    /// </summary>
    /// <param name="authoritative">Field definitions for the stage the submission targets — server-resolved, never client-supplied.</param>
    /// <param name="submitted">Form values submitted by the client, keyed by field key.</param>
    /// <param name="hiddenFieldKeys">
    /// Field keys belonging to a component whose <c>showWhen</c> evaluated to hidden right now —
    /// skipped entirely, the same as a field hidden via the older <see cref="FieldRenderPayload.ConditionalOn"/>
    /// pair. A required field that's genuinely not visible must never block submission.
    /// </param>
    public static FieldValidationResult Validate(
        ...
        IReadOnlySet<string>? hiddenFieldKeys = null)
    {
        ...
        foreach (var field in authoritative)
        {
            ...
            // Skip fields hidden by the calc-driven showWhen mechanism entirely.
            if (hiddenFieldKeys is not null && hiddenFieldKeys.Contains(field.FieldKey))
            {
                continue;
            }

            // Skip hidden conditional fields (the older scalar ConditionalOn/VisibleWhen pair).
        ...
        return errors.Count == 0
            ? FieldValidationResult.Pass()
            : FieldValidationResult.Fail(errors);
    }
    ...
    private static string? ValidateConstraints(FieldRenderPayload field, string raw)
    {
        ...
        if (!string.IsNullOrWhiteSpace(field.Pattern))
        {
            ...
            {
                // Pattern from the blueprint is too complex or causes catastrophic backtracking
