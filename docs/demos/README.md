...
| `bulk-data-review.webm` | An NJF operations user submitting a monthly contributions CSV to SafetyNet Underwriting, reviewing only the rows that need attention (never the whole file), correcting one in place, resubmitting the whole corrected file through a genuine loop back to the same external system, and finishing while a non-blocking warning is still on record. Written companion: [bulk-data-review-walkthrough.md](./bulk-data-review-walkthrough.md). | `Wayfinder.ReferenceApp.Tests/tests/demo/bulk-data-review-demo.spec.ts` |
